  ...
  module RoomsHelper
    ...
    def submit_room_button_tag
      button_tag class: "btn btn--reversed txt-large center", type: "submit" do
        image_tag("campfire/check.svg", aria: { hidden: "true" }, size: 20) +
    ...
    def room_display_name(room, for_user: Campfire::Current.user)
      if room.direct?
        room.users.without(for_user).map(&:name).to_sentence.presence || for_user&.name
      else
        room.name
      end
    end
    ...
    private
      ...
      def composer_data_actions
        ...
        [ drag_and_drop_actions, trix_attachment_actions, remaining_actions ].join(" ")
      end
  end
end